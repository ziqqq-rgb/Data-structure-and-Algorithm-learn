...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        min_price = float('inf') #inf is infinity, if put 0, then our minimum will always zero, so we put infinity, so that any number will be less than it on the first round
        max_profit = 0
